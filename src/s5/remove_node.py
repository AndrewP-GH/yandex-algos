...
#  алгоритмическая сложность будет
#  - O(log(n)) в среднем и O(n) в худшем случае,
#  - O(H), где H - высота дерева.
#
# Т.к. алгоритм проводит операции над нодами рекурсивно и не использует дополнительных коллекций, на каждом шаге
#   рекурсии необходимо O(1) памяти, что приводит к итоговой сложности по памяти O(H), где H - высота дерева.

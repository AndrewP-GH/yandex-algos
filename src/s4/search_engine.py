# Отчет 67649610: https://contest.yandex.ru/contest/24414/run-report/67649610/
# Алгоритм работы:
...
#   а m - количество слов в каждом документе.
...
#   Алгоритмическая сложность O(l), l - количество уникальных слов в запросе, этап 2 будут выполняться k раз (по кол-ву
#  запросов), поэтому итоговая сложность будет O(l*k)
#   Тут я не учитываю сложность сортировки (она в среднем O(n*log(n))), где n - число документов, в которых встречаются
#   слова, т.к. есть ощущение, что O(l) > O(n*log(n))
#   Сложность по памяти - O(l), l - количество уникальных слов, но это сложность на каждом шаге (для каждого поиска),
# и после обработки запроса ее можно переиспользовать
# 3) Вывести на экран результат поиска
# Итоговая алгоритмическая сложность O(n*m + l*k), т.к. все вычисления происходят за O(1),
...
def search_engine():
    ...
    for i in range(m):
        ...
        print(' '.join(map(str, [x[0] for x in result])))

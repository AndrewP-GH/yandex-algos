...
#   в данный документ.
#   Алгоритмическая сложность O(n*m*s), т.к. поиск и вставка слова осуществляются за O(1), но вычисление хэша происходит
#   за O(k), где s - размер слова, n - количество документов, m - количество слов в каждом документе.
#   Сложность по памяти - O(m*s), m - количество уникальных слов.
...
#   Алгоритмическая сложность поиска и подсчета будет O(l*s), где l - количество уникальных слов в запросе,
#   этап 2 будет выполняться k раз (по кол-ву запросов), поэтому итоговая сложность будет O(k(l*s + n*log(n))),
...
#   Сложность по памяти - O(l*s), l - количество уникальных слов, но это сложность на каждом шаге (для каждого поиска),
#   и после обработки запроса память можно переиспользовать.
# 3) Вывести на экран результат поиска.
#   Итоговая алгоритмическая сложность O(n*m*s + k(l*s + n*log(n))), т.к. все вычисления происходят за O(1),
#   а число документов, запросов, слов и их длина  неизвестны.
# Сложность по памяти - O(s(m + l)), m - количество уникальных слов в документах, l - среднее количество уникальны слов
# в запросе, s - длина слова

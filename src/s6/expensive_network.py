...
# - вставка в словарь: O(m)
# - проход по словарю и вставка ключей в set: O(m)
# - удаление вершины из посещенных и проверка, что ребро соединяет с непосещенной: O(n + m)
# - вставки в бинарную кучу: O(m*log(n))

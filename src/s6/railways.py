...
#   и строго убывающий по номерам городов путь B -> A, создавая таким образом цикл. Тогда карта с такими путями
#   не будет оптимальной, потому что по обоим типам дорог можно построить путь из A в B (один путь строго по дорогам
#   первого типа, и второй - строго по дорогам второго типа), а значит, карта не является оптимальной.
#
